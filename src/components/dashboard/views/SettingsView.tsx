...
const SettingsView = ({ onNavigate }: { onNavigate?: (view: string) => void }) => {
  ...
  const handleExportCSVZip = async () => {
    ...
    try {
      const isEmpresarioAccount = profile.role === 'empresario';
      const [profileRes, favRes, bookingsRes, convsRes] = await Promise.all([
        supabase.from('profiles').select('*').eq('user_id', user.id).maybeSingle(),
        supabase.from('favorites').select('*').eq('user_id', user.id),
        // Un booking pertenece al usuario tanto si lo creó (empresario) como si es
...
bookings.csv
  Historial completo de Flash Bookings, tanto los que has creado como
  empresario como aquellos en los que te contrataron como profesional.
...
resumen_anual.csv
  Resumen ejecutivo con KPIs de actividad, estadísticas de red y
  desglose mensual de bookings.
...
Protección de Datos (RGPD / GDPR). Para el derecho al olvido (Art. 17),
usa la opción "Eliminar cuenta" en Ajustes > Zona de peligro.
