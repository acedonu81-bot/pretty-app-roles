...
const article = { '@context': 'https://schema.org', '@type': 'Article', headline: 'Catering para comuniones en España: precio por persona y qué incluye (2026)', description: 'Cuánto cuesta el catering de una comunión en España. Precios por persona, formatos de menú y cómo elegir el servicio correcto para tu celebración.', datePublished: '2026-05-07',
  dateModified: '2026-05-25', author: { '@type': 'Person', name: 'Daniel', jobTitle: 'Fundador de XPEAK', url: 'https://xpeak.es/autor/daniel' }, publisher: { '@type': 'Organization', name: 'XPEAK', url: 'https://xpeak.es', logo: { '@type': 'ImageObject', url: 'https://xpeak.es/favicon.png' } }, image: 'https://xpeak.es/og-image.jpg', url: 'https://xpeak.es/blog/catering-comuniones-precio-persona' };
    ...
    <>
      ...
      <div className="min-h-screen" style={{ background: '#ffffff', color: '#111' }}>
        <nav className="flex items-center justify-between px-4 sm:px-6 py-4 max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3"><a href="/blog" className="text-xs font-bold" style={{ color: '#3d3d4e' }}>Blog</a><a href="/auth" className="px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all hover:scale-105" style={{ background: 'linear-gradient(90deg,#B45309,#B8941E)', color: '#000' }}>Unirse</a></div>
        </nav>
        <main className="max-w-3xl mx-auto px-4 sm:px-6 pt-8 pb-20">
          ...
          <div className="mb-8">
            ...
            <p className="text-base leading-relaxed" style={{ color: '#3d3d4e' }}>El catering es la partida más importante del presupuesto de una comunión. Con estos precios reales y este desglose de formatos, podrás comparar presupuestos sin sorpresas.</p>
          ...
          <div className="space-y-10">
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  ...
                  <div key={c.label} className="p-3 rounded-xl text-center" style={{ background: 'rgba(180,83,9,0.06)', border: '1px solid rgba(180,83,9,0.15)' }}>
                    ...
                    <p className="text-lg font-black" style={{ color: '#B45309' }}>{c.precio}</p>
                    <p className="text-[0.6rem]" style={{ color: '#666' }}>{c.nota}</p>
                  </div>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="overflow-x-auto rounded-xl" style={{ border: '1px solid rgba(0,0,0,0.08)' }}>
                <table className="w-full text-xs">
                  <thead><tr style={{ background: 'rgba(180,83,9,0.08)', borderBottom: '1px solid rgba(0,0,0,0.06)' }}><th className="text-left px-4 py-3 font-bold">Formato</th><th className="px-4 py-3 font-bold text-right">Precio/persona</th><th className="px-4 py-3 font-bold text-left hidden sm:table-cell">Ideal para</th></tr></thead>
                  <tbody>{FORMATOS.map((row, i) => (<tr key={row.formato} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.015)', borderBottom: '1px solid rgba(0,0,0,0.04)' }}><td className="px-4 py-3 font-medium">{row.formato}</td><td className="px-4 py-3 text-right font-bold" style={{ color: '#B45309' }}>{row.precio}</td><td className="px-4 py-3 hidden sm:table-cell" style={{ color: '#3d3d4e' }}>{row.ideal}</td></tr>))}</tbody>
                </table>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                {EXTRAS.map(e => (
                  <div key={e.extra} className="flex items-center justify-between px-3 py-2.5 rounded-lg" style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.06)' }}>
                    <span className="text-xs" style={{ color: '#222' }}>{e.extra}</span>
                    <span className="text-xs font-bold flex-shrink-0 ml-3" style={{ color: '#B45309' }}>{e.precio}</span>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={t.tip} className="p-4 rounded-xl" style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.06)' }}>
                    <p className="text-sm font-bold mb-1" style={{ color: '#B45309' }}>💡 {t.tip}</p>
                ...
                {faqStructured.mainEntity.map(q => (
                  <details key={q.name} className="rounded-xl overflow-hidden" style={{ border: '1px solid rgba(0,0,0,0.07)' }}>
                    <summary className="flex items-center justify-between px-4 py-3.5 cursor-pointer text-sm font-bold list-none" style={{ background: 'rgba(0,0,0,0.02)' }}>
                      {q.name}
                      <span className="text-xs ml-3 flex-shrink-0" style={{ color: '#B45309' }}>+</span>
                    </summary>
            ...
            <section className="rounded-2xl p-6 text-center" style={{ background: 'rgba(180,83,9,0.06)', border: '1px solid rgba(180,83,9,0.2)' }}>
              <p className="text-xs font-bold uppercase tracking-widest mb-2" style={{ color: '#B45309' }}>¿Buscas catering para tu comunión?</p>
              ...
              <a href="/contratar-catering" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-sm transition-all hover:scale-105" style={{ background: 'linear-gradient(90deg,#B45309,#B8941E)', color: '#000' }}>
