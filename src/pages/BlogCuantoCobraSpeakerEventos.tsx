  ...
  return (
    <>
      ...
      <div className="min-h-screen" style={{ background: '#ffffff', color: '#111' }}>
              ...
              <div className="space-y-2">
                {PRECIOS.map((row, i) => (
                  <div key={row.concepto} className="flex items-center justify-between p-4 rounded-xl" style={{ background: i % 2 === 0 ? 'rgba(0,0,0,0.025)' : 'rgba(0,0,0,0.015)', border: '1px solid rgba(0,0,0,0.04)' }}>
                    ...
                    <span className="text-xs font-black ml-4 shrink-0" style={{ color: '#2563EB' }}>{row.precio}</span>
                  </div>
                ))}
              </div>
            </section>
        ...
        <FooterPublic />
        <BlogScrollCTA role="speaker_pro" storageKey="xpeak_scrollcta_speaker_precio_v2" articlePath="/blog/cuanto-cobra-un-speaker-de-eventos" />
      </div>
    </>
  );
}
