  ...
  return (
    <>
      ...
      <div className="min-h-screen" style={{ background: '#ffffff', color: 'rgba(22,20,18,0.88)' }}>
        {/* Nav */}
        <nav className="flex items-center justify-between px-4 sm:px-6 py-4 max-w-5xl mx-auto" style={{ borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          <div className="flex items-center gap-3">
            <a href="/" className="text-xs font-semibold hidden sm:flex items-center gap-1 transition-all hover:opacity-70" style={{ color: 'rgba(22,20,18,0.5)' }}>← Inicio</a>
            <a href="/blog" className="text-xs font-bold hidden sm:block" style={{ color: 'rgba(22,20,18,0.5)' }}>Blog</a>
            <a href="/auth?role=profesional" className="text-xs font-bold hidden sm:block" style={{ color: 'rgba(22,20,18,0.5)' }}>Soy profesional</a>
        ...
        <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-8 pb-20">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-2xl sm:text-4xl font-black tracking-tight leading-tight mb-3" style={{ color: 'rgba(22,20,18,0.92)' }}>{config.title}</h1>
            <p className="text-sm leading-relaxed max-w-xl" style={{ color: 'rgba(22,20,18,0.55)' }}>{config.subtitle}</p>
          </div>
            ...
            {ALL_ROLES.map(r => (
              <a key={r.slug} href={`/directorio/${r.slug}`}
                ...
                style={r.slug === (rol ?? 'dj')
                  ...
                  : { background: '#f5f4f0', color: 'rgba(22,20,18,0.6)', border: '1px solid rgba(0,0,0,0.08)' }
                }>{r.label}</a>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            <span className="text-xs font-bold self-center" style={{ color: 'rgba(22,20,18,0.4)' }}>Ciudad:</span>
            {CITIES.map(c => (
              <button key={c} onClick={() => setCity(c)}
                className="px-3 py-1 rounded-lg text-xs font-medium transition-all"
                style={c === city
                  ? { background: 'rgba(212,175,55,0.12)', color: '#B8941E', border: '1px solid rgba(212,175,55,0.35)' }
                  : { background: '#f5f4f0', color: 'rgba(22,20,18,0.6)', border: '1px solid rgba(0,0,0,0.07)' }
                }>{c}</button>
            ))}
            <span className="text-xs self-center ml-auto" style={{ color: 'rgba(22,20,18,0.35)' }}>
          ...
          {/* Grid de perfiles */}
          {fetchError && (
            <div className="p-12 rounded-2xl text-center" style={{ background: '#f9f8f6', border: '1px solid rgba(0,0,0,0.07)' }}>
              <p className="text-sm font-bold mb-2">No se pudo cargar el directorio</p>
              <p className="text-xs mb-4" style={{ color: 'rgba(22,20,18,0.5)' }}>Comprueba tu conexión y vuelve a intentarlo.</p>
          ...
          {!fetchError && loading && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="rounded-2xl p-4 animate-pulse" style={{ background: '#f5f4f0', border: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          {!fetchError && !loading && profiles.length === 0 && (
            <div className="p-12 rounded-2xl text-center" style={{ background: '#f9f8f6', border: '1px solid rgba(0,0,0,0.07)' }}>
              ...
              <p className="text-xs mb-4" style={{ color: 'rgba(22,20,18,0.5)' }}>Prueba con otra ciudad o ve al directorio completo.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {profiles.map(p => (
                <div key={p.user_id} className="rounded-2xl overflow-hidden flex flex-col bg-white"
                  style={{
                    border: (p as any).is_early_adopter ? '2px solid rgba(96,165,250,0.7)' : '1px solid rgba(0,0,0,0.08)',
                    boxShadow: (p as any).is_early_adopter ? '0 0 20px rgba(96,165,250,0.2), 0 2px 12px rgba(0,0,0,0.06)' : '0 2px 12px rgba(0,0,0,0.06)',
                  }}>

                  {/* Foto */}
                  <a href={`/p/${p.user_id}`} className="block relative" style={{ aspectRatio: '4/3', overflow: 'hidden' }}>
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    <a href={`/p/${p.user_id}`} className="block hover:opacity-70 transition-opacity">
                      <p className="text-base font-black leading-tight mb-0.5" style={{ color: 'rgba(22,20,18,0.92)' }}>{p.display_name}</p>
                      {p.specialty && (
                        <p className="text-xs font-semibold mb-1" style={{ color: '#D4AF37' }}>{p.specialty}</p>
                      )}
                    </a>
                    <div className="flex items-center gap-1.5 mb-2">
                      <div className="flex gap-0.5">
                        {[1,2,3,4,5].map(s => (
                          <span key={s} style={{ color: s <= Math.round((p.score ?? 0) / 20) ? '#D4AF37' : 'rgba(0,0,0,0.12)', fontSize: 11 }}>★</span>
                        ))}
                      </div>
                      {p.is_verified && (
                        <span className="text-[0.65rem]" style={{ color: 'rgba(22,20,18,0.4)' }}>Verificado</span>
                      )}
                    </div>
                    {p.zone && (
                      <p className="text-xs flex items-center gap-1 mb-2" style={{ color: 'rgba(22,20,18,0.5)' }}>
                        <MapPin size={10} />{p.zone.split(',')[0]}
                      </p>
                    )}
                    {p.bio && (
                      <p className="text-xs leading-relaxed mb-3 line-clamp-2" style={{ color: 'rgba(22,20,18,0.55)' }}>
                        "{p.bio.slice(0, 100)}{p.bio.length > 100 ? '…' : ''}"
                      </p>
                    )}
                    <div className="mt-auto flex gap-2">
                      <a href={`/p/${p.user_id}`}
                        className="flex-1 text-center py-2.5 rounded-xl text-xs font-bold transition-all hover:bg-black/5"
          ...
          <div className="mt-16 p-6 rounded-2xl text-center" style={{ background: 'linear-gradient(135deg,rgba(212,175,55,0.08),rgba(249,115,22,0.06))', border: '1px solid rgba(212,175,55,0.2)' }}>
            <p className="text-sm font-black mb-1" style={{ color: 'rgba(22,20,18,0.92)' }}>¿Eres profesional y quieres aparecer aquí?</p>
            <p className="text-xs mb-4" style={{ color: 'rgba(22,20,18,0.55)' }}>
