...
const faqStructured = {
  ...
  const tdStyle = { padding: '10px 14px', borderBottom: '1px solid rgba(0,0,0,0.06)', color: '#111', fontSize: '0.88rem', verticalAlign: 'top' as const };
  const thStyle = { padding: '10px 14px', textAlign: 'left' as const, fontSize: '0.72rem', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.5px', color: '#DB2777', borderBottom: '1px solid rgba(219,39,119,0.3)', background: 'rgba(219,39,119,0.05)' };
    ...
    <>
      ...
      <div className="min-h-screen" style={{ background: '#ffffff', color: '#111' }}>
        ...
        <article className="max-w-3xl mx-auto px-4 sm:px-6 pt-6 pb-20 sm:pb-24">
          ...
          {/* Breadcrumb */}
          <div className="flex items-center gap-1.5 text-xs mb-6" style={{ color: '#555' }}>
            <a href="/" style={{ color: '#555' }} className="hover:text-[#8A6D0F] transition-colors">Inicio</a>
            <ChevronRight size={12} />
            <a href="/blog" style={{ color: '#555' }} className="hover:text-[#8A6D0F] transition-colors">Blog</a>
            <ChevronRight size={12} />
            <span style={{ color: '#333' }}>Cuánto cuesta una boda</span>
          </div>
          ...
          <header className="mb-10">
            ...
            <div className="flex items-center gap-4 mt-5 text-xs" style={{ color: '#555' }}>
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {RANGOS.map(r => (
                <div key={r.tipo} className="rounded-xl p-5" style={{ background: 'rgba(0,0,0,0.03)', border: `1px solid rgba(0,0,0,0.07)` }}>
                  <p className="text-xs font-bold uppercase tracking-wider mb-1" style={{ color: '#333' }}>{r.invitados}</p>
          ...
          <section className="mb-12">
            ...
            <div className="overflow-x-auto rounded-xl" style={{ border: '1px solid rgba(0,0,0,0.07)' }}>
                    ...
                    <tr key={i}>
                      <td style={tdStyle}>
                        <div className="flex items-center gap-2">
                          <p.icon size={14} style={{ color: '#DB2777', flexShrink: 0 }} />
                ...
                <tbody>
                  {CIUDADES.map((c, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ ...tdStyle, fontWeight: 700, color: '#DB2777' }}>{c.rango}</td>
                      <td style={{ ...tdStyle, color: '#444' }}>{c.nota}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
              {[
                ...
                <div key={d.tipo} className="rounded-xl p-4" style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.07)' }}>
                  ...
                  <p className="text-lg font-black mb-1" style={{ color: '#DB2777' }}>{d.precio}</p>
                  <p className="text-xs" style={{ color: '#444' }}>{d.desc}</p>
                </div>
              ))}
            </div>
            <div className="rounded-xl p-4 flex items-start gap-3" style={{ background: 'rgba(219,39,119,0.06)', border: '1px solid rgba(219,39,119,0.2)' }}>
              ...
              <p className="text-sm" style={{ color: '#222' }}>
                ...
                <a href="/contratar-dj" className="font-bold underline" style={{ color: '#DB2777' }}>Ver DJs disponibles →</a>
              </p>
            </div>
          </section>
          ...
          <section className="mb-12">
            ...
            <div className="overflow-x-auto rounded-xl mb-4" style={{ border: '1px solid rgba(0,0,0,0.07)' }}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i}>
                      ...
                      <td style={{ ...tdStyle, fontWeight: 700, color: '#DB2777' }}>{r.precio}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="rounded-xl p-4 flex items-start gap-3" style={{ background: 'rgba(219,39,119,0.06)', border: '1px solid rgba(219,39,119,0.2)' }}>
              ...
              <p className="text-sm" style={{ color: '#222' }}>
                ...
                <a href="/blog/cuantos-camareros-necesito-para-mi-boda" className="font-bold underline" style={{ color: '#DB2777' }}>
              ...
              {[
                ...
                <div key={c.num} className="flex gap-4 rounded-xl p-4" style={{ background: 'rgba(0,0,0,0.02)', border: '1px solid rgba(0,0,0,0.06)' }}>
                  ...
                  <div>
                    ...
                    <p className="text-sm" style={{ color: '#444' }}>{c.desc}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className="mb-12">
            ...
            <div className="space-y-4">
              {FAQ.map((f, i) => (
                <div key={i} className="rounded-xl p-5" style={{ background: 'rgba(0,0,0,0.03)', border: '1px solid rgba(0,0,0,0.07)' }}>
                  <p className="font-bold mb-2">{f.q}</p>
                  <p className="text-base leading-relaxed" style={{ color: '#333' }}>{f.a}</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <div className="rounded-2xl p-6 sm:p-8 text-center" style={{ background: 'linear-gradient(135deg, rgba(219,39,119,0.1) 0%, rgba(219,39,119,0.04) 100%)', border: '1px solid rgba(219,39,119,0.25)' }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <a href="/contratar-camareros"
                className="px-5 py-3 rounded-xl font-bold text-sm transition-all hover:scale-105"
                style={{ background: 'rgba(0,0,0,0.06)', border: '1px solid #777', color: '#111' }}>
                Contratar camareros
              </a>
            </div>
          </div>
          ...
          <section className="mt-10 mb-4">
            <a href="/presupuesto-boda"
              ...
              style={{ background: 'rgba(219,39,119,0.05)', border: '1px solid rgba(219,39,119,0.2)' }}>
          ...
          <section className="mt-8">
            <p className="text-xs font-bold uppercase tracking-wider mb-4" style={{ color: '#666' }}>Artículos relacionados</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <a key={a.href} href={a.href}
                  ...
                  <ChevronRight size={14} style={{ color: '#DB2777', marginTop: 2, flexShrink: 0 }} />
                  <div>
                    <p className="text-xs font-bold mb-0.5" style={{ color: '#DB2777' }}>{a.cat}</p>
                    <p className="text-sm font-semibold" style={{ color: '#111' }}>{a.title}</p>
                  </div>
                </a>
              ))}
            </div>
