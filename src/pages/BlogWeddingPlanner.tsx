        ...
        <main className="max-w-3xl mx-auto px-4 sm:px-6 pt-8 pb-20">
          ...
          <div className="space-y-10">
            ...
            <section>
              ...
              <p className="text-sm leading-relaxed mb-4" style={{ color: 'rgba(255,255,255,0.75)' }}>El wedding planner no es solo "alguien que decora". Sus funciones varían mucho según el paquete contratado, pero en la planificación integral suelen incluir:</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={f.title} className="p-4 rounded-xl flex gap-3" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={d.title} className="p-5 rounded-xl" style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${d.c}30` }}>
                    ...
                    <div className="space-y-1">
                      {d.contras.map(c => <p key={c} className="text-xs flex gap-2" style={{ color: 'rgba(255,255,255,0.75)' }}><span style={{ color: '#ff5f56' }}>✗</span>{c}</p>)}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section>
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={item.factor} className="flex gap-4 p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <section>
              ...
              <div className="space-y-4">
                {CHECKLIST.map((fase) => (
                  <div key={fase.fase} className="p-4 rounded-xl" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
                    <h3 className="text-xs font-black mb-3 uppercase tracking-wide" style={{ color: '#D4AF37' }}>{fase.fase}</h3>
                    <ul className="space-y-1.5">
                      {fase.tareas.map(t => (
                        <li key={t} className="text-xs flex items-start gap-2" style={{ color: 'rgba(255,255,255,0.75)' }}>
                          <span className="w-3.5 h-3.5 rounded border flex-shrink-0 mt-0.5" style={{ borderColor: 'rgba(212,175,55,0.3)' }} />
                          {t}
                        </li>
                      ))}
                    </ul>
            ...
            <section>
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div key={q} className="flex gap-3 p-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.04)' }}>
                    <span className="text-xs font-black w-5 flex-shrink-0" style={{ color: '#D4AF37' }}>{i + 1}.</span>
                    <p className="text-xs" style={{ color: 'rgba(255,255,255,0.8)' }}>{q}</p>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section>
              ...
              <p className="text-sm leading-relaxed mb-4" style={{ color: 'rgba(255,255,255,0.75)' }}>Depende del tipo de boda. Hay dos escenarios claros donde el wedding planner se amortiza solo:</p>
                ...
                <div className="p-5 rounded-xl" style={{ background: 'rgba(34,197,94,0.04)', border: '1px solid rgba(34,197,94,0.15)' }}>
                  ...
                  <ul className="space-y-2">
                    {['Más de 80 invitados', 'Presupuesto superior a 20.000€', 'Boda en destino fuera de tu ciudad', 'Ambos trabajáis y no tenéis tiempo', 'Boda en más de un día o en varios espacios'].map(p => (
                      <li key={p} className="text-xs flex gap-2" style={{ color: 'rgba(255,255,255,0.75)' }}><span style={{ color: '#22c55e' }}>→</span>{p}</li>
                    ))}
                  </ul>
                </div>
                <div className="p-5 rounded-xl" style={{ background: 'rgba(255,95,86,0.04)', border: '1px solid rgba(255,95,86,0.15)' }}>
                  ...
                  <ul className="space-y-2">
                    {['Boda íntima de menos de 40 personas', 'Presupuesto ajustado (menos de 10.000€)', 'Finca con coordinadora incluida', 'Familia muy implicada en la organización', 'Os gusta organizar y tenéis tiempo libre'].map(p => (
                      <li key={p} className="text-xs flex gap-2" style={{ color: 'rgba(255,255,255,0.75)' }}><span style={{ color: '#ff5f56' }}>→</span>{p}</li>
                    ))}
                  </ul>
                </div>
              </div>
              <div className="p-4 rounded-xl" style={{ background: 'rgba(212,175,55,0.06)', border: '1px solid rgba(212,175,55,0.15)' }}>
                <p className="text-xs leading-relaxed" style={{ color: 'rgba(255,255,255,0.8)' }}>
                  <strong style={{ color: '#D4AF37' }}>Dato clave:</strong> Un buen wedding planner negocia descuentos con proveedores habituales que suelen cubrir entre el 30% y el 70% de su propio honorario. En bodas con presupuesto de 30.000€, un ahorro del 10% en proveedores ya supera los 3.000€ — lo que cuesta el planificador.
                </p>
              </div>
            </section>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <a key={a.href} href={a.href} className="p-4 rounded-xl transition-all hover:scale-[1.02]" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', display: 'block', textDecoration: 'none' }}>
                    <span className="text-[0.6rem] font-bold uppercase tracking-wider mb-1 block" style={{ color: '#D4AF37' }}>{a.tag}</span>
                    <span className="text-xs font-bold" style={{ color: 'rgba(255,255,255,0.85)' }}>{a.title}</span>
                  </a>
                ))}
              </div>
            </section>
