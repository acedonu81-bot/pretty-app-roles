-- Añade user_id a admin_activity para poder cruzar cada fila con los emails
-- que se le mandaron a esa persona (email_send_log, migración anterior).
-- "quiero ver los correos que se le manda... paso por encima con una
-- interrogación y veo el correo que se ha mandado" (12 sep 2026).
...
SELECT * FROM (
  ...
  SELECT
    ...
    b.professional_user_id AS user_id
  ...
  SELECT 'oferta_evento'::text AS tipo,
    ...
    (CASE WHEN e.client_user_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
          THEN e.client_user_id::uuid ELSE NULL END) AS user_id
  ...
  SELECT 'respuesta_oferta'::text AS tipo,
    ...
    er.professional_user_id AS user_id
  ...
  SELECT 'contratacion'::text AS tipo,
    ...
    er2.professional_user_id AS user_id
...
RETURNS TABLE(tipo text, cuando timestamp with time zone, quien text, detalle text, lugar text, contacto text, ref text, pendiente boolean, que_pide text, cuando_evento text, user_id uuid)
LANGUAGE plpgsql
